...
/**
 * Internal library functions for local_grpcalendarimport.
 *
...
function local_grpcalendarimport_create_event(array $row, bool $skipduplicates, int $rownum): array {
    ...
    if (empty($eventname)) {
        return ['row' => $rownum, 'status' => 'error',
            'message' => get_string('error_missingname', 'local_grpcalendarimport')];
    }
    if ($courseid <= 0) {
        return ['row' => $rownum, 'status' => 'error',
            'message' => get_string('error_invalidcourseid', 'local_grpcalendarimport')];
    }
    if ($groupid <= 0) {
        return ['row' => $rownum, 'status' => 'error',
            'message' => get_string('error_invalidgroupid', 'local_grpcalendarimport')];
    }
    if ($timestart <= 0) {
        return ['row' => $rownum, 'status' => 'error',
            'message' => get_string('error_invalidtimestart', 'local_grpcalendarimport')];
    }

    // Verify course exists.
    if (!$DB->record_exists('course', ['id' => $courseid])) {
        return ['row' => $rownum, 'status' => 'error',
            'message' => get_string('error_coursenotfound', 'local_grpcalendarimport', $courseid)];
    }

    // Verify group exists and belongs to this course.
    if (!$DB->record_exists('groups', ['id' => $groupid, 'courseid' => $courseid])) {
        return ['row' => $rownum, 'status' => 'error',
            'message' => get_string('error_groupnotfound', 'local_grpcalendarimport',
                (object)['groupid' => $groupid, 'courseid' => $courseid])];
    }
    ...
    if ($skipduplicates) {
        ...
        if ($exists) {
            return ['row' => $rownum, 'status' => 'skip',
                'message' => get_string('status_skip_msg', 'local_grpcalendarimport', $eventname)];
        }
    }
    ...
    try {
        ...
        return [
            'row'     => $rownum,
            'status'  => 'ok',
            'message' => get_string('status_ok_msg', 'local_grpcalendarimport',
                (object)['name' => $eventname, 'id' => $calendarevent->id]),
        ];
    } catch (\Throwable $e) {
        return ['row' => $rownum, 'status' => 'error',
            'message' => get_string('error_exception', 'local_grpcalendarimport', $e->getMessage())];
    }
}
